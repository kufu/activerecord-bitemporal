...
RSpec.describe "Association" do
  ...
  describe "BTDM has many BTDM" do
    ...
    describe "sync transaction_from(created_at) in create" do
      let!(:employee1) { company.employees.new(name: "Jane") }
      let!(:employee2) { company.employees.new(name: "Homu") }
      let!(:employee3) { company.employees.new(name: "Mami", transaction_from: "2016/01/15") }
      let!(:employee4) { company.employees.new(name: "Mado", created_at: "2020/01/01") }
      let!(:employee5) { company.employees.new(name: "Mado", created_at: "2020/01/01", transaction_from: "2016/01/15") }
      ...
      context "#new to #save" do
        ...
        context "saving" do
          subject { -> { company.save! } }
          it { is_expected.to change { employee1.transaction_from } }
          it { is_expected.to change { employee2.transaction_from } }
          it { is_expected.not_to change { employee3.transaction_from } }
          it { is_expected.to change { employee3.created_at } }
          it { is_expected.to change { employee4.transaction_from } }
          it { is_expected.not_to change { employee4.created_at } }
          it { is_expected.to change { employee5.transaction_from } }
          it { is_expected.not_to change { employee5.created_at } }
        end

        context "saved" do
          subject { company.transaction_from }
          before { company.save! }
          it { is_expected.to eq(employee1.transaction_from).and(eq(employee1.created_at)) }
          it { is_expected.to eq(employee2.transaction_from).and(eq(employee2.created_at)) }
          it { expect(employee1.transaction_from).to eq employee2.transaction_from }
          it { expect(employee3.transaction_from).to eq("2016/01/15").and(eq(employee3.created_at)) }
          it { expect(employee4.transaction_from).to eq("2020/01/01").and(eq(employee4.created_at)) }
          it { expect(employee5.transaction_from).to eq("2020/01/01").and(eq(employee5.created_at)) }
        end

        context "company with transaction_from" do
          let(:company) { Company.new(name: "Company", transaction_from: "2018/12/25") }
          subject { company.transaction_from }
          before { company.save! }
        ...
        context "nested associastion" do
          let(:transaction_from) { "2019/1/1" }
          let!(:company) { Company.new(transaction_from: transaction_from) }
          let!(:employee) { company.employees.new }
          let!(:address) { employee.address = Address.new }
          before { company.save! }
          it { expect(company.transaction_from).to  eq(transaction_from).and(eq(company.created_at)) }
          it { expect(employee.transaction_from).to eq(transaction_from).and(eq(employee.created_at)) }
          it { expect(address.transaction_from).to  eq(transaction_from).and(eq(address.created_at)) }

          context "address with `transaction_from`" do
            let!(:address) { employee.address = Address.new(transaction_from: "2010/1/1") }
            it { expect(address.transaction_from).to eq("2010/1/1").and(eq(address.created_at)) }
          end
          context "address with `created_at`" do
            let!(:address) { employee.address = Address.new(created_at: "2010/1/1") }
            it { expect(address.created_at).to eq("2010/1/1").and(eq(address.transaction_from)) }
          end
          context "address with `created_at` and `transaction_from`" do
            let!(:address) { employee.address = Address.new(created_at: "2010/1/1", transaction_from: "2020/1/1") }
            it { expect(address.created_at).to eq("2010/1/1").and(eq(address.transaction_from)) }
          end
        end
      end
      ...
      context "#create to #save" do
        ...
        context "saved" do
          before { company.save }
          subject { company.transaction_from }
          it { is_expected.not_to eq employee1.transaction_from }
          it { is_expected.not_to eq employee2.transaction_from }
          it { expect(employee1.transaction_from).not_to eq employee2.transaction_from }
          it { expect(employee1.created_at).not_to eq employee2.created_at }
          it { expect(employee3.transaction_from).to eq("2016/01/15").and(eq(employee3.created_at)) }
          it { expect(employee4.transaction_from).to eq("2020/01/01").and(eq(employee4.created_at)) }
          it { expect(employee5.transaction_from).to eq("2020/01/01").and(eq(employee5.created_at)) }
        end
